...
class AllGemsCategoryAdapter (val context: Context,
                              private var gemsList: List<Gems>,
                              private val onItemClick: (String) -> Unit

) : RecyclerView.Adapter<AllGemsCategoryAdapter.categories_ViewHolder>() {
    class categories_ViewHolder (itemView: View): RecyclerView.ViewHolder(itemView) {
        val name = itemView.findViewById<TextView>(R.id.all_categories_name)
        val image=itemView.findViewById<ImageView>(R.id.all_categories_img)

    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): categories_ViewHolder {
        val view= LayoutInflater.from(context).inflate(R.layout.view_holder_all_gems_category,parent,false)
